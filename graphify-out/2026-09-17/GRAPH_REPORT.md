# Graph Report - tesis  (2026-09-17)

## Corpus Check
- 243 files · ~253,980 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3883 nodes · 7750 edges · 186 communities (161 shown, 23 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 693 edges (avg confidence: 0.88)
...
- make_chessboard.py
- DualFlightLogger
- Sesgo constante por camara invisible a la tolerancia
- TestMemoriaEmpuje
...
Cohesion: 0.06
Nodes (51): ConjuntoSincronizado, MultiCameraSync, Emparejamiento temporal de varias cámaras sin sincronización por hardware. Las…, Los frames de un mismo instante, con lo que quedó fuera y por qué., `{nombre: frame}` para quien sólo quiera las imágenes., Salud del emparejamiento a lo largo de una sesión. No es depuración: con seis…, Empareja los frames de varias cámaras por cercanía temporal. Acepta cualquier…, Toma el frame nuevo de cada stream, si lo hay. **Una lectura por stream y por… (+43 more)
...
### Community 2 - "Gesture"
Cohesion: 0.09
Nodes (24): evento(), FakeFlight, FakeFollow, Verifica la traduccion de gesto a orden de vuelo. Sin radio ni motores. Lo…, Con mocap las ordenes van en el marco de la SALA y `rumbo` es hacia donde mira…, `confirmed=False` significa «no ejecutar». Es la regla del contrato., Doble del backend de vuelo: registra ordenes, no toca hardware., La nariz del dron 90 grados a TU izquierda: tu ADELANTE queda, visto desde el… (+16 more)
...
Cohesion: 0.09
Nodes (37): Body3DRecognizer, Convierte landmarks de MediaPipe en `GestureEvent` confirmados. Mantiene dos…, La escala corporal ya es estable., _brazo(), FakeLandmark, pose(), ndarray, Comprueba el vocabulario 3D sin camara, sin dron y sin MediaPipe. Construye… (+29 more)
...
Cohesion: 0.09
Nodes (26): Lanzador del control Cruz multiproceso de dos drones por cámara., airborne_states(), camera_loop(), draw_panel(), execute_command(), main(), parse_args(), Namespace (+18 more)
...
### Community 23 - "mapeo3d/pose/__init__.py"
Cohesion: 0.20
...
Cohesion: 0.08
Nodes (32): Lanzador canónico del control de un Crazyflie por cámara y gestos., al_marco_del_dron(), al_marco_del_mundo(), _altura(), _aplicar(), bucle(), _comando_ejecutado(), crear_reconocedor() (+24 more)
...
Cohesion: 0.08
Nodes (15): arm_if_supported(), Arma explícitamente en cflib reciente; conserva compatibilidad antigua., Detiene la prueba si el firmware no detecta el Flow deck v2., require_flow_deck(), FlowDroneController, Crazyflie, MotionCommander, Bloquea hasta que termina el preflight; lanza `RuntimeError` si falló. (+7 more)
...
Cohesion: 0.09
Nodes (27): app_calibracion(), app_pose3d(), _args(), _mundo_sintetico(), fixture, Namespace, Pruebas de las apps de `apps/`, sin cámaras y sin pantalla. Motivo de existir:…, La ruta de captura entera, con la forma de las esquinas verificada. (+19 more)

### Community 35 - "dtw_distancia"
Cohesion: 0.13
Nodes (23): Gestos dinamicos por DTW (aplausos y secuencias), dtw_distancia(), ndarray, Reconocimiento de gestos dinamicos por comparacion con plantillas (DTW). Es el…, Distancia DTW entre dos trayectorias `(N, D)` y `(M, D)`. Se normaliza por el…, Umbral que mejor separa repeticiones del gesto de material sin el. Returns:…, umbral_por_separacion(), ndarray (+15 more)

### Community 36 - "medir_ptz.py"
Cohesion: 0.18
Nodes (15): diferencia_angular(), main(), Mide la velocidad angular y la repetibilidad del pan/tilt de la camara. Por que…, `a - b` en grados, teniendo en cuenta que el pan da la vuelta en 360., Da un paso y devuelve `(grados_recorridos, segundos_de_motor)`. El tiempo se…, un_paso(), Construye el cliente a partir de la URL RTSP de la misma camara. Evita repetir…, resumen_capacidades() (+7 more)

### Community 37 - "CameraMarkerFollower"
Cohesion: 0.13
Nodes (12): CameraMarkerFollower, FollowUnavailable, _fresh(), RuntimeError, Seguimiento relativo de un marker Robotat para controladores de cámara. Este…, Fija el ancla de cada dron a `FOLLOW_RADIUS_M` del marker. Con `level=True` el…, Posición actual del marker 65, o `FollowUnavailable` si no es reciente., Velocidad para un backend `send_velocity_world_setpoint`. (+4 more)
...
Cohesion: 0.16
Nodes (5): PanelDosRobotat, Any, Tecla o botón de dirección pulsado: empieza a mover ese dron., Tecla o botón soltado: si ese dron no tiene otra tecla, frena., Ventana con dos columnas de botones y el teclado dual, en modo fluido.
...
Cohesion: 0.09
Nodes (8): HighlevelCameraMarkerRuntime, Convierte objetivos del marker en pasos validados del backend., Cancela objetivos sin esperar al cierre del receptor MQTT., selected_keys(), Backend, Follower, Adaptación high-level del seguimiento, sin radio ni MQTT., RuntimeTests

### Community 45 - "Toma"
Cohesion: 0.07
Nodes (51): entre_personas(), evaluar(), main(), por_sujeto(), ndarray, rasgos_2d(), rasgos_3d(), Compara 3D contra 2D sobre las tomas grabadas. Sin camara y sin dron. Responde… (+43 more)
...
Cohesion: 0.06
Nodes (73): Ajustes, centro_torso(), decidir_con_gesto(), Orden, ndarray, Politica de seguimiento: donde esta la persona -> hacia donde y cuan rapido.…, Lo que hay que pedirle al motor. `codigo == PARAR` es detenerse., Parametros del lazo. Los de fabrica son deliberadamente suaves. (+65 more)
...
Cohesion: 0.13
Nodes (8): angle_delta_deg(), MarkerInput, Adaptador headless del marker joystick; no abre radios ni controla drones. Lee…, Diferencia angular en [-180, 180], segura al cruzar ±180°., Zona muerta amplia y rampa suave desde 12° hasta 28°., tilt_to_speed(), MarkerInputTests, Traduccion de marker con receptor falso: sin conexion MQTT ni radio.
...
### Community 55 - "PoseDetector"
Cohesion: 0.07
Nodes (12): Vocabulario 3D de cuerpo entero, mas los dos gestos de mano del marker., ReconocedorCuerpo, HandsSource, Fuente de manos para sesiones duales; la coordinacion recibe gestos filtrados., HandTracker, Encapsula MediaPipe Hands. Devuelve: - frame anotado - landmarks de la primera…, Devuelve todas las manos detectadas, conservando su handedness., Devuelve landmarks, lateralidad y confianza de todas las manos. (+4 more)
...
Cohesion: 0.20
Nodes (15): hold_hover(), hover(), main(), parse_args(), positive_float(), Crazyflie, MotionCommander, Namespace (+7 more)

### Community 58 - "VueloRobotat"
Cohesion: 0.10
Nodes (10): Un Crazyflie sobre el Robotat como backend de un controlador por cámara., El preflight ya es síncrono; existe por compatibilidad con Flow Deck., Los parámetros se aplican en el preflight desde `Opciones`; devuelve lo…, Despegando o aterrizando: no se aceptan velocidades., Altura sobre el origen del preflight, o `None` sin pose., Velocidad en el marco del Robotat, m/s, mientras dure el gesto., La cámara sigue viva pero no pide movimiento: frenar si se movía., Un paso de seguimiento del marker 65: velocidad hacia el ancla. El ancla la… (+2 more)
...
### Community 62 - "test_dron_robotat.py"
Cohesion: 0.09
Nodes (19): Estado, fence_velocity(), ramp_velocity(), Qué hacer en vuelo: ("aterrizar" | "cortar", motivo), o None si todo va bien.…, Acerca `actual` a `deseada` sin superar `accel · dt` por paso., Velocidad horizontal del marco del mundo al del cuerpo (x nariz, y izquierda)., Quita de `vel` lo que sacaría a `pose` de la geocerca. Horizontal: si la…, Lo que el panel necesita mostrar. Copia inmutable por consulta. (+11 more)

### Community 63 - "DroneUnit"
Cohesion: 0.11
Nodes (13): DroneUnit, mqtt_timestamp_is_older(), parse_mqtt_timestamp(), Pose, Crazyflie, datetime, Estado de un Crazyflie sobre el Robotat: pose MQTT, extpos, telemetría EKF y…, Prepara el EKF para posición externa con el commander high-level activo. (+5 more)

### Community 64 - "Latencia"
Cohesion: 0.14
...
Cohesion: 0.06
Nodes (30): 2026-09-17, 18:45: seguir y orbitar el marker con los gestos dinámicos, 2026-09-17, 18:59: seguimiento bien; el aplauso se leyó como círculo, Cuarto vuelo: `--ganancias mitad --velocidad 0.25` (15:26, Dron 2), Cámara de dos drones con el controlador nuevo (19:34): funciona, Dron 1 (15:06 y 15:07): no despega por batería, Dron 1 con batería nueva (15:15): tampoco levanta, Dual de las 18:55: el Dron 2 se apaga por batería, no por el Robotat, Empuje de hover medido (17:01 y 17:03, Dron 2) (+22 more)

### Community 66 - "analizar_sesion_robotat.py"
Cohesion: 0.08
Nodes (50): analyze_session(), cargar(), _direccion_de_evento(), _direccion_por_desplazamiento(), escribir_resumen(), figura_actitud(), figura_altura(), figura_bateria_empuje() (+42 more)
...
Cohesion: 0.07
Nodes (58): main(), medir_umbral(), Construye el banco de plantillas dinamicas desde las tomas grabadas. Uso, desde…, Deja fuera a cada persona y separa las distancias de acierto y de fallo. El…, Hacia donde mira el cuerpo respecto de la camara, en grados.…, yaw_camara(), formato_matriz(), formato_metricas() (+50 more)

### Community 69 - "DronSimulado"
Cohesion: 0.08
Nodes (20): DronSimulado, goto_duration_s(), Opciones, Misma interfaz que `DronRobotat`, sin radio ni broker. Para `--dry-run`., Espera a que termine la orden en curso. False si no terminó a tiempo., Duración de un `go_to` proporcional a lo que hay que recorrer., Ajustes elegidos en la línea de órdenes., opciones() (+12 more)
...
Cohesion: 0.10
Nodes (28): _angulo_deg(), _angulo_sagital(), angulos_a_direcciones(), brazo_que_senala(), BrazoActivo, clasificar(), _confianza(), diagnostico() (+20 more)
...
### Community 78 - "gui_pdf_capture.py"
Cohesion: 0.21
...
Cohesion: 0.18
Nodes (7): HttpTests, Integracion HTTP y sesiones: todo simulado, sin radios, MQTT ni camara., ThreadingHTTPServer, finite_json(), main(), PanelServer, Panel HTTP de experimentos. La logica de vuelo pertenece a controllers/.
...
Cohesion: 0.18
Nodes (7): MocapReceiver, Pose, Ultima pose si tiene menos de `timeout_s`; si no, `None`., Suscriptor MQTT seguro para una sola pose de ROBOTAT., main(), OrientationChecker, Comprobador visual del marker ROBOTAT. No conecta ni arma el dron.
...
Cohesion: 0.18
Nodes (7): Aplica los límites de un paso; lanza `DronError` si se exceden., Geocerca horizontal alrededor del origen y rango vertical absoluto., validate_step(), validate_target(), parametrize, TestObjetivo, TestPaso

### Community 85 - "ReconocedorVocabulario"
Cohesion: 0.16
Nodes (9): _confundible_con_aplauso(), _orbitar_marker(), Dinamicos (DTW) + estaticos + paro, con dos modos excluyentes. Por frame hace…, Un frame de decision: `(aviso para el panel, pedir emergencia)`. Quien dice si…, True si `det` no es `aplaudir` pero `aplaudir` quedó demasiado cerca., Un paso de órbita alrededor del marker 65; sólo el backend `robotat` sabe…, ReconocedorVocabulario, fixture (+1 more)

### Community 86 - "HandGestureDetector"
Cohesion: 0.09
Nodes (17): HandGestureDetector, Detector de gestos de mano basado en MediaPipe Hands. Vocabulario implementado:…, Escala aproximada de la mano. Se usa la distancia muñeca -> MCP del dedo medio., Determina dedos extendidos usando distancias desde la muñeca. Esto es más…, Pulgar extendido: - punta más lejos de la muñeca que la articulación IP/MCP - y…, Estima si la mano apunta hacia arriba o hacia abajo. Se usa el promedio de las…, Confirma DESPEGAR y ATERRIZAR solo si se sostienen durante cierto tiempo. Esto…, Suavizado por mayoría para evitar parpadeos por detecciones aisladas. (+9 more)

### Community 87 - "test_marker_follow_level.py"
Cohesion: 0.27
Nodes (8): pose(), Ancla del seguidor a la altura del marker (`activate(level=True)`)., ReceptorFalso, seguidor(), test_ancla_nivelada_con_dron_encima_del_marker(), test_ancla_nivelada_ignora_la_altura_del_dron(), test_ancla_tridimensional_sigue_igual(), test_marker_position()

### Community 88 - "mapeo3d/pose/detector.py"
Cohesion: 0.16
...
### Community 94 - "dron_robotat.py"
Cohesion: 0.04
Nodes (57): Reduccion de ~21900 a ~14500 lineas sin perder funcionalidad de vuelo, Exclusión de flujo óptico y ToF en Robotat, configure_estimator(), Operaciones sobre un enlace Crazyflie que todos los controladores repiten.…, Pulso de `kalman.resetEstimation`., Prepara el Crazyflie para volar con posición externa del Robotat. Excluye la…, Corta los motores de uno o varios Crazyflie repitiendo el stop setpoint. Se…, reset_kalman() (+49 more)
...
### Community 97 - "SeguidorFalso"
Cohesion: 0.17
Nodes (4): Imita `CameraMarkerFollower`: ancla fija y objetivo configurable., SeguidorFalso, TestOrbita, TestSeguirMarker
...
### Community 100 - "probar_vocabulario.py"
Cohesion: 0.12
Nodes (15): AjustesPTZ, La camara sigue al operador con su pan/tilt, como en el probador., bucle(), dibujar_panel(), _direccion_mas_cercana(), Lineas, main(), _panel_dinamicos() (+7 more)
...
### Community 104 - "test_grafica_comandos.py"
Cohesion: 0.21
...
### Community 105 - "Comandos para probar los programas"
Cohesion: 0.14
...
Cohesion: 0.07
Nodes (33): bucle(), _contador_compacto(), dibujar_panel(), _falta_dos_manos(), main(), Counter, _que_falta(), Prueba del vocabulario 3D. Sin dron, sin radio y sin cflib. No importa… (+25 more)
...
### Community 116 - "ptz/__init__.py"
Cohesion: 0.21
Nodes (8): ErrorPTZ, _parsear_respuesta(), RuntimeError, Cliente PTZ para camaras Amcrest / Dahua sobre su API HTTP CGI. Solo depende de…, La camara no acepto la peticion PTZ., `clave=valor` por linea -> diccionario., Aplicar ordenes PTZ sin bloquear el bucle de vision. Por que existe este modulo…, Seguimiento de la persona con una camara IP pan/tilt. Dos piezas separadas a…

### Community 117 - "BancoDinamico"
Cohesion: 0.17
Nodes (9): cargar_banco(), Banco DTW del vocabulario, o `None` si el reconocedor no lo usa. Se comprueba…, BancoDinamico, Path, Plantillas y umbral de aceptacion, medidos sobre material real., Gestos que el banco puede emitir. `GESTO_RECHAZO` no es uno., Umbral que se le exige a `gesto`: el suyo si lo tiene, si no el global., `(gesto, distancia, margen, distancias por gesto)`. `gesto` es `None` si la… (+1 more)

### Community 120 - "wrap_deg"
Cohesion: 0.21
Nodes (8): centro_geocerca(), plan_hold_segment(), Objetivo, yaw y duración del siguiente tramo continuo. La dirección se…, Simulación: la pose avanza un paso de `FLUID_LOOKAHEAD_S` por llamada., Ángulo equivalente en [-180, 180)., Centro de la geocerca: el fijado en las opciones o, si no hay, el origen del…, wrap_deg(), TestTramoContinuo

### Community 121 - "describir_error"
Cohesion: 0.24
Nodes (10): BaseException, describir_error(), Convierte la excepcion en algo que diga que hay que arreglar. Los tres fallos…, _http_error(), Las credenciales van en la cabecera Digest; que no se cuelen aqui., test_el_mensaje_nunca_lleva_credenciales(), test_otros_errores_no_se_tragan(), test_un_401_habla_de_credenciales_no_de_red() (+2 more)
...
Cohesion: 0.09
Nodes (36): analyze_session(), main(), number(), Path, Genera gráficas y un resumen de la última sesión de dos drones., Crea figuras PDF para un CSV y devuelve la carpeta de salida., session_day(), session_output_dir() (+28 more)
...
### Community 141 - "normalize.py"
Cohesion: 0.06
Nodes (39): Un paso del seguimiento. `True` si el frame se tomo con la camara girando. Va…, carpeta_de_hoy(), bucle(), como_hacerlo(), construir_guion(), dibujar(), encuadre(), main() (+31 more)
...
### Community 147 - "Anclaje al marco Robotat con blanco rastreado por el MoCap"
Cohesion: 0.26
Nodes (13): AGENTS.md — contrato para agentes de mapeo3d, Distincion: OptiTrack/MoCap vs camaras IP, Marcadores ArUco fijos medidos una vez, Anclaje al marco Robotat con blanco rastreado por el MoCap, Bola de color en el centroide de un cluster de marcadores, Tablero ChArUco rastreado como cuerpo rigido, LED-ring deck de Bitcraze como baliza, Interferencia IR 850 nm contra el OptiTrack (+5 more)
...
### Community 149 - "opciones_camara"
Cohesion: 0.20
Nodes (7): `VueloRobotat`: la interfaz que espera el controlador por cámara, sin hardware., test_opciones_camara_centro_geocerca(), TestInterfaz, TestOpciones, vuelo(), opciones_camara(), `Opciones` del dron para la cámara: preajuste de ganancias más `--param`.

### Community 151 - "config/cameras.local.yaml (plantilla cameras.example.yaml)"
Cohesion: 0.28
...
### Community 152 - "Montaje de camaras en las esquinas del Robotat"
Cohesion: 0.22
...
### Community 155 - "Registro"
Cohesion: 0.25
Nodes (4): _columnas_dinamico(), Columnas del CSV para la detección dinámica cerrada en este frame., Una fila por frame en `results/data/control_camara_dron1/<dia>/<sesion>.csv`., Registro
...
### Community 157 - "RobotatCruzBackend"
Cohesion: 0.09
Nodes (13): opciones_desde_args(), Any, Distancia entre las dos poses del mocap, o None si falta alguna., Aterriza a los dos drones si en vuelo se acercan a menos de `minimo_m`., Una comprobación; devuelve la separación. Se puede llamar sin hilo., Uno o dos `DronRobotat` con la interfaz del backend de la cruz., Velocidad ahora y parada programada; una orden nueva reprograma la parada., Paso del protocolo (dx, dy, dz, dyaw) como pulso de velocidad fluida. (+5 more)

### Community 158 - "Pipeline de gestos por vision: diseno (docs/agents/gesture_pipeline.md)"
Cohesion: 0.07
Nodes (35): AGENTS.md (contrato raiz del repositorio), Separacion canal continuo (navegacion) / canal de eventos (comandos de estado), config.py mezcla tres subsistemas (deuda tecnica), Dos sistemas de camaras separados: MoCap (dron) vs camaras IP (operador), Baseline 1: MediaPipe + DTW, Baseline 2: MediaPipe + HMM, Leave-One-Subject-Out (LOSO) / validacion estricta por sujeto, Modelo posterior: LSTM/GRU sobre landmarks normalizados (+27 more)

### Community 159 - "GraficaDeComandos"
Cohesion: 0.29
Nodes (5): GraficaDeComandos, Path, Acumula `(t, comando)` durante la sesión y guarda la figura al cerrar. Args:…, Una muestra por frame: qué comando se atendió y en qué estado., Escribe PNG y PDF. Devuelve la ruta del PNG, o `None` si no hubo nada que…

### Community 160 - "EstimadorDeEscala"
Cohesion: 0.13
...
### Community 168 - "control_camara_dron1.py (controlador único por cámara)"
Cohesion: 0.05
Nodes (60): Seguridad de hardware, contains_id(), Descubre el tópico MQTT que corresponde a una ID de rigid body ROBOTAT. No abre…, Busca la ID tanto como número como texto dentro del JSON recibido., Marker joystick: rigid body 64 en mocap/all, Protocolo de seguimiento del marker Robotat 65, marker_follow.py (seguimiento del marker 65), marker_input.py (marker 64 como joystick) (+52 more)

### Community 169 - "make_chessboard.py"
Cohesion: 0.40
Nodes (4): main(), parse_args(), Namespace, Genera un tablero de ajedrez para calibración, a escala exacta, en PDF. El…

### Community 170 - "DualFlightLogger"
Cohesion: 0.32
Nodes (3): Namespace, DualFlightLogger, test_connect_deja_los_dos_listos()

### Community 171 - "Sesgo constante por camara invisible a la tolerancia"
Cohesion: 0.33
Nodes (6): Extrinsecos estereo (intrinsecos fijos, tablero al 8 %), Sesgo constante por camara invisible a la tolerancia, Amcrest IP4M-1041B (x6, objetivo), Diagnostico RTSP: 401 vs 404, Red para las seis Amcrest, Wi-Fi 2.4 GHz contra el Crazyradio

### Community 173 - "Validacion en tres niveles"
Cohesion: 0.50
Nodes (4): Verificacion obligatoria del frame rate real, Nakano et al. (2020) Evaluation of 3D Markerless Motion Capture Accuracy Using OpenPose, Reproducibilidad de sesion (calibracion, stats, config), Validacion en tres niveles

### Community 174 - "PoseFrame3D (contrato de salida)"
Cohesion: 0.15
Nodes (16): Pipeline capture → pose → triangulation → io, Salida MQTT topico vision/pose3d, Triangulacion independiente del backend de pose, Marco de coordenadas del Robotat como salida unica, Paquete src/mapeo3d con lanzadores en apps/, PoseFrame3D (contrato de salida), Herramientas externas: Caliscope, aniposelib, Pose2Sim, Homografia del suelo: dominio de validez (+8 more)

### Community 177 - "test_triangulation.py"
Cohesion: 0.13
...
### Community 186 - "experiment_session.py"
Cohesion: 0.10
Nodes (26): highlevel_delta, controllers/joystick/marker_follow.py (seguimiento marker 65), Adaptador del seguimiento de marker al backend high-level de cámara., Sesion de experimento headless: coordina backend, entradas y resultados., Conversion de gestos filtrados a comandos high-level, sin camara., Registro y graficas de sesiones; mide tiempos del software, no de la radio., T-003 Extpos a 60 Hz y yaw al EKF, T-004 Go_to sin solapamiento y métrica de oscilación (+18 more)

### Community 187 - "Punto3D"
Cohesion: 0.50
...
### Community 195 - ".orbit_marker"
Cohesion: 0.25
Nodes (5): Marca vertical con etiqueta: emergencias, aterrizajes forzados., plan_orbita(), Punto del círculo en `theta` (a la altura del centro) y velocidad tangencial., Un paso de órbita alrededor del marker 65 (gesto `circulo`). El objetivo está…, Deja en el CSV del dron, una vez por segundo, adónde se le pide ir.

### Community 207 - "sin_hardware"
Cohesion: 0.50
Nodes (4): fixture, MonkeyPatch, Sustituye MotionCommander y el armado por dobles durante cada prueba., sin_hardware()

### Community 208 - "GestureEvent"
Cohesion: 0.11
Nodes (12): Gate de engagement obligatorio antes de cualquier comando, evento_de_mano(), Traduce la etiqueta del detector de mano al contrato `GestureEvent`. El…, Gestos de una mano en 2D. Prototipo anterior al vocabulario corporal., El mismo evento, pero sin orden: para pintar y registrar, no para mover., Alimenta los tres canales con un frame. Devuelve el evento estatico para el…, ReconocedorManos, _sin_gesto() (+4 more)
...
## Knowledge Gaps
- **197 isolated node(s):** `DummyLandmarks`, `form`, `names`, `directions`, `gestures` (+192 more)
...
- **Why does `Refactorizacion de septiembre de 2026 (docs/agents/refactor_2026-09.md)` connect `dron_robotat.py` to `AGENTS.md: contrato canónico para agentes`, `body_3d_rules.py`, `probar_gestos_3d.py`, `BridgeError`, `marker_input.py`, `FlowDroneController`, `Pipeline de gestos por vision: diseno (docs/agents/gesture_pipeline.md)`, `DroneUnit`?**
  _High betweenness centrality (0.096) - this node is a cross-community bridge._
- **Why does `canonicalizar()` connect `test_canonical.py` to `test_robust.py`, `normalize.py`, `mapeo3d/pose/__init__.py`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `rasgos_de_sesion()` connect `test_canonical.py` to `body_3d_rules.py`, `mapeo3d/pose/__init__.py`?**
