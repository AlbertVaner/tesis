# Graph Report - tesis  (2026-09-16)

## Corpus Check
- 243 files · ~248,708 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3825 nodes · 7623 edges · 190 communities (167 shown, 21 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 683 edges (avg confidence: 0.88)
...
- grafica_comandos.py
- ReglaParo
- control_dos_drones_camara_multiprocessing.py
- external/mapeo3d/
- capture/__init__.py
- MocapFeed
- _torso
...
- CameraStream
- fake_cf
- test_control_dos_drones_robotat.py
- test_apps.py
- dtw_distancia
- ptz/__init__.py
...
- Marker joystick: rigid body 64 en mocap/all
...
- 2026-09-17 Nucleo Robotat en shared.md
- .en_movimiento
- Mapa de documentacion para agentes (docs/agents/README.md)
- test_triangulation.py
- T-006 Controlador de un dron sobre el Robotat desde cero.md
- 2026-09-16.md
- Receiver

## God Nodes (most connected - your core abstractions)
1. `Command` - 99 edges
2. `BridgeError` - 55 edges
3. `Gesture` - 51 edges
4. `DronRobotat` - 46 edges
5. `Seguidor` - 43 edges
6. `Ajustes` - 41 edges
7. `HardwareBackend` - 39 edges
8. `dron_hw()` - 37 edges
9. `DronError` - 36 edges
10. `SimulatedBackend` - 36 edges
...
Cohesion: 0.10
Nodes (19): evento(), FakeFlight, FakeFollow, Verifica la traduccion de gesto a orden de vuelo. Sin radio ni motores. Lo…, Con mocap las ordenes van en el marco de la SALA y `rumbo` es hacia donde mira…, `confirmed=False` significa «no ejecutar». Es la regla del contrato., Doble del backend de vuelo: registra ordenes, no toca hardware., La nariz del dron 90 grados a TU izquierda: tu ADELANTE queda, visto desde el… (+11 more)

### Community 3 - "FlowCruzBackend"
Cohesion: 0.16
Nodes (6): FlowCruzBackend, Any, Manda una velocidad y programa su parada. No bloquea la interfaz., Uno o dos Crazyflies con Flow deck, con la interfaz del backend de la cruz., FlowDroneConfig, StateCallback
...
Cohesion: 0.14
Nodes (6): HighLevelButtonsApp, Any, Mueve un objetivo concreto; se usa por botones y por teclado dual., Gira un objetivo concreto sin moverlo., Enter: despega lo que esté en el suelo, aterriza lo que esté en vuelo. Decide…, LabelFrame

### Community 7 - "flowdeck_cruz_backend.py"
Cohesion: 0.15
Nodes (19): main(), r"""Panel de botones para un solo Crazyflie: la interfaz de la cruz en modo de…, main(), parse_args(), Namespace, r"""Interfaz Python por botones para comparar control high-level en dos drones.…, build_backend(), Dos Crazyflies con Flow deck vistos como el backend high-level de la cruz.… (+11 more)

### Community 8 - "ChessboardSpec"
Cohesion: 0.09
Nodes (29): ChessboardSpec, corner_movement(), corner_span(), draw_corners(), find_corners(), find_corners_preview(), _normalizar(), ndarray (+21 more)
...
Cohesion: 0.07
Nodes (36): cabecera_autorizacion(), describe(), _digest(), escanear(), _parsear(), puerto_abierto(), Sondeo RTSP de bajo nivel, para saber POR QUÉ no conecta una cámara.…, Envía `DESCRIBE` y devuelve `(estado, motivo, cabeceras)`. Si la cámara… (+28 more)
...
Cohesion: 0.09
Nodes (42): Enum, Gesture, Contrato entre la visión y los controladores. El vocabulario vive aquí y en…, Vocabulario único, común al detector 2D y al 3D., Body3DRecognizer, Convierte landmarks de MediaPipe en `GestureEvent` confirmados. Mantiene dos…, La escala corporal ya es estable., _brazo() (+34 more)

### Community 14 - "triangulation/__init__.py"
Cohesion: 0.13
Nodes (25): camera_center(), project(), ndarray, Triangulación lineal de N vistas por DLT. Este módulo no sabe qué es RTSP ni…, Proyecta puntos 3D con una matriz `(3, 4)`. Devuelve `(K, 2)`., Error de reproyección en píxeles, por punto: `(K,)`. Es lo que permite al…, Ángulo entre los rayos de dos cámaras hacia un punto, en grados. Es lo que…, Centro óptico de una cámara a partir de su matriz `(3, 4)`. (+17 more)

### Community 15 - "grafica_comandos.py"
Cohesion: 0.11
Nodes (21): agrupar(), _color(), _figura(), Muestra, orden_de_filas(), Gráfica de tiempo contra comandos para los controladores por cámara. Cada…, Tramos consecutivos con la misma `clave(muestra)`: `(valor, inicio, fin)`. Cada…, Un tramo por cada racha de `(comando, confirmado)` iguales. (+13 more)
...
### Community 17 - "control_dos_drones_camara_multiprocessing.py"
Cohesion: 0.09
Nodes (27): Lanzador del control Cruz multiproceso de dos drones por cámara., airborne_states(), camera_loop(), draw_panel(), execute_command(), main(), parse_args(), Namespace (+19 more)
...
Cohesion: 0.09
Nodes (22): CameraConfig, CaptureConfig, Config, ConfigNotFound, find_config(), FileNotFoundError, Path, Carga de la configuración de cámaras. La configuración real vive en… (+14 more)

### Community 20 - "MocapFeed"
Cohesion: 0.06
Nodes (29): _component(), MocapFeed, parse_message(), parse_timestamp(), datetime, quaternion_euler_deg(), quaternion_yaw_deg(), Alimentación del Robotat a un Crazyflie: MQTT → frames distintos → extpos.… (+21 more)
...
Cohesion: 0.09
Nodes (15): CameraIntrinsics, Path, Rectifica coordenadas de píxel, devolviéndolas en píxeles. Es la versión de…, Resultado de una calibración intrínseca., Campo de visión horizontal, derivado del ajuste. Sustituye a la medición con…, Metros de alto que abarca el cuadro por cada metro de distancia. Es el número…, Distancia mínima para que un objeto de ese alto quepa entero. Supone la cámara…, Path (+7 more)

### Community 23 - "GestureEvent"
Cohesion: 0.15
Nodes (9): evento_de_mano(), Traduce la etiqueta del detector de mano al contrato `GestureEvent`. El…, Gestos de una mano en 2D. Prototipo anterior al vocabulario corporal., El mismo evento, pero sin orden: para pintar y registrar, no para mover., Alimenta los tres canales con un frame. Devuelve el evento estatico para el…, ReconocedorManos, _sin_gesto(), GestureEvent (+1 more)

### Community 24 - "VelocityIntent"
Cohesion: 0.11
Nodes (19): Separacion canal continuo (navegacion) / canal de eventos (comandos de estado), Canal continuo, en el marco del cuerpo del operador. Unidades normalizadas…, VelocityIntent, Un dron imaginario que responde al vocabulario como lo haria el real. La logica…, DINAMICO o ESTATICO; el aplauso conmuta., hover, SEGUIR, ALEJARSE, ORBITAR o MANUAL., Gesto dinamico o de estado. `(accion, motivo)`; motivo vacio si se ejecuto., Canal continuo. Se llama cada frame con la direccion confirmada. (+11 more)

### Community 25 - "test_control_vocabulario.py"
Cohesion: 0.08
Nodes (29): deteccion(), evento(), FakeControl, FakeFlight, FakeFollow, _Landmark, landmarks_en(), paso() (+21 more)

### Community 26 - "test_extrinsics.py"
Cohesion: 0.08
Nodes (31): calibrate_stereo(), _errores_por_par(), ParesIncoherentes, RuntimeError, Los pares no describen una sola geometría: una cámara se movió., Error de reproyección de cada par, con la pose estimada del tablero. `poses_a`…, Estima la pose de la cámara B respecto de la A. Los intrínsecos se fijan…, extrinsecos() (+23 more)

### Community 27 - "control_camara_dron1.py"
Cohesion: 0.07
Nodes (39): Lanzador canónico del control de un Crazyflie por cámara y gestos., al_marco_del_dron(), al_marco_del_mundo(), _altura(), _aplicar(), bucle(), cargar_banco(), _comando_ejecutado() (+31 more)
...
Cohesion: 0.09
Nodes (14): Marcas de tiempo de llegada, no de camara, Captura sin acumular latencia (hilo lector, solo ultimo frame), _ahora(), CameraStream, Devuelve `(frame, timestamp)` si hay uno nuevo, o `None`. El timestamp es de…, Como `read()`, pero devuelve el último frame aunque ya se haya leído. Útil para…, (ancho, alto) reportados por el decodificador, o None si no hay conexión., fps que declara el stream. NO confiar en este número: ver `stats`. (+6 more)
...
### Community 33 - "test_control_dos_drones_robotat.py"
Cohesion: 0.36
Nodes (7): args(), _drones(), Panel de dos drones sobre el controlador nuevo: opciones y supervisor, sin Tk., test_opciones_dual_por_defecto(), test_param_explicito_para_los_dos(), test_supervisor_aterriza_si_se_acercan(), test_supervisor_no_actua_con_uno_solo()

### Community 34 - "test_apps.py"
Cohesion: 0.09
Nodes (24): _args(), _mundo_sintetico(), Namespace, Pruebas de las apps de `apps/`, sin cámaras y sin pantalla. Motivo de existir:…, La ruta de captura entera, con la forma de las esquinas verificada., Con un mínimo imposible no debe capturar nada automáticamente. Es la…, 33 landmarks con caderas en el origen, como los devuelve MediaPipe., El módulo tiene que cargar aunque MediaPipe no esté: la importación es perezosa… (+16 more)

### Community 35 - "dtw_distancia"
Cohesion: 0.18
Nodes (14): dtw_distancia(), ndarray, Distancia DTW entre dos trayectorias `(N, D)` y `(M, D)`. Se normaliza por el…, ndarray, Un movimiento continuo de dos landmarks, como matriz `(MUESTRAS, 6)`., La misma persona hace el mismo gesto un 20-30 % mas rapido entre repeticiones.…, Sin banda, DTW alinea un pulso con cualquier cosa y devuelve una distancia…, test_dos_gestos_distintos_quedan_lejos() (+6 more)

### Community 36 - "ptz/__init__.py"
Cohesion: 0.13
Nodes (20): diferencia_angular(), main(), Mide la velocidad angular y la repetibilidad del pan/tilt de la camara. Por que…, `a - b` en grados, teniendo en cuenta que el pan da la vuelta en 360., Da un paso y devuelve `(grados_recorridos, segundos_de_motor)`. El tiempo se…, un_paso(), ErrorPTZ, RuntimeError (+12 more)

### Community 37 - "CameraMarkerFollower"
Cohesion: 0.14
Nodes (11): CameraMarkerFollower, FollowUnavailable, _fresh(), RuntimeError, Seguimiento relativo de un marker Robotat para controladores de cámara. Este…, Fija el ancla de cada dron a `FOLLOW_RADIUS_M` del marker. Con `level=True` el…, Velocidad para un backend `send_velocity_world_setpoint`., Velocidad para MotionCommander, expresada en el marco del dron. (+3 more)

### Community 38 - "DronRobotat"
Cohesion: 0.09
Nodes (18): DronError, DronRobotat, RuntimeError, Un envío de velocidad. Devuelve False cuando ya devolvió el mando., Rampa en cero: el high-level recupera el mando con un `go_to` corto a la…, Corta el hilo de velocidad y devuelve el mando antes de otra orden., Empuje medio del hover y el `thrustBase` que conviene la próxima vez., Corta motores. Enclava: después sólo queda cerrar el programa. (+10 more)

### Community 39 - "StereoExtrinsics"
Cohesion: 0.10
Nodes (12): ndarray, Path, Pose de la cámara B respecto de la A., Distancia entre los dos centros ópticos, en metros., Ángulo entre los ejes ópticos de las dos cámaras. No es el ángulo de…, Matrices `(3, 4)` de las dos cámaras, con origen en la cámara A. Es lo único…, Ángulo entre rayos hacia un punto, visto desde las dos cámaras., StereoExtrinsics (+4 more)

### Community 40 - "mapeo3d/pose/__init__.py"
Cohesion: 0.08
Nodes (43): a_escena(), camara_orbital(), _color_de(), dibujar_esqueleto_2d(), dibujar_esqueleto_3d(), dibujar_rejilla(), proyectar(), ndarray (+35 more)

### Community 41 - "PanelDosRobotat"
Cohesion: 0.15
Nodes (4): PanelDosRobotat, Tecla o botón de dirección pulsado: empieza a mover ese dron., Tecla o botón soltado: si ese dron no tiene otra tecla, frena., Ventana con dos columnas de botones y el teclado dual, en modo fluido.

### Community 42 - "check_pose3d.py"
Cohesion: 0.13
Nodes (19): deque, _corto(), estabilidad(), guardar(), informe_final(), main(), _panel_3d(), _panel_texto() (+11 more)

### Community 43 - "DualStepKeysMixin"
Cohesion: 0.18
Nodes (8): disable_button_keyboard_focus(), DualStepKeysMixin, Event, Envuelve una acción de una pulsación con la guarda del panel., Un paso de giro. Los paneles sin yaw no lo implementan., Reserva Espacio para el dron, no para los botones de Tkinter., Un paso por pulsación. El panel implementa `_key_step` y `_keys_enabled`., Misc

### Community 44 - "HighlevelCameraMarkerRuntime"
Cohesion: 0.07
Nodes (15): highlevel_delta, controllers/joystick/marker_follow.py (seguimiento marker 65), HighlevelCameraMarkerRuntime, Adaptador del seguimiento de marker al backend high-level de cámara., Convierte objetivos del marker en pasos validados del backend., Cancela objetivos sin esperar al cierre del receptor MQTT., selected_keys(), Sesion de experimento headless: coordina backend, entradas y resultados. (+7 more)

### Community 45 - "Toma"
Cohesion: 0.09
Nodes (36): cargar(), cargar_todas(), guardar(), _limpio(), Path, Guardado y lectura de tomas de gestos. Una **toma** es una repeticion de un…, Todas las tomas de una carpeta, en orden de nombre. Busca tambien en…, Cuantas tomas hay de cada gesto, persona y orientacion. (+28 more)

### Community 46 - "Pose"
Cohesion: 0.21
Nodes (3): Pose, Ultima pose si tiene menos de `timeout_s`; si no, `None`., MarkerInputTests
...
Cohesion: 0.09
Nodes (52): Ajustes, Parametros del lazo. Los de fabrica son deliberadamente suaves., Maquina de estados del lazo de seguimiento. Uso:: seg = Seguidor() orden =…, Politica cuando la camara ademas reconoce gestos. **El gesto tiene prioridad,…, Seguidor, _codigo(), Politica de seguimiento PTZ. Sin camara, sin red y sin MediaPipe. Lo que se…, El sobrepaso se acota limitando cuanto dura cada movimiento. (+44 more)
...
### Community 50 - "dron_hw"
Cohesion: 0.11
Nodes (14): alimentar(), dron_hw(), FakeCf, Un pico de 2.99 V de una muestra no aterriza; medio segundo si., Mete un frame de mocap y una estimacion EKF coincidentes., Con vz = 0 el objetivo se queda: el lazo de posición debe traer al dron., La altura mandada no se adelanta más de la correa a la estimada., test_altura_fluida_con_correa() (+6 more)

### Community 51 - "test_calibration.py"
Cohesion: 0.11
Nodes (31): calibrate(), Ajusta los intrínsecos a partir de las esquinas detectadas. Args:…, intrinsecos(), K_real(), fixture, ndarray, Pruebas de `calibration/` sin cámaras. Se proyectan puntos de un tablero con…, Aunque una vista llegue como (N,2), calibrate debe funcionar. Es la segunda… (+23 more)

### Community 52 - "app.js"
Cohesion: 0.18
Nodes (19): action(), command(), config(), directions, drawTrajectory(), field(), fillConfig(), fmt() (+11 more)

### Community 53 - "marker_input.py"
Cohesion: 0.18
Nodes (7): angle_delta_deg(), MarkerInput, Adaptador headless del marker joystick; no abre radios ni controla drones. Lee…, Diferencia angular en [-180, 180], segura al cruzar ±180°., Zona muerta amplia y rampa suave desde 12° hasta 28°., tilt_to_speed(), Traduccion de marker con receptor falso: sin conexion MQTT ni radio.
...
### Community 56 - "mapeo3d/pose/detector.py"
Cohesion: 0.16
...
Cohesion: 0.11
Nodes (29): load_config(), Aplica la precedencia documentada en el docstring del módulo., Carga y valida la configuración de cámaras., cam-1" -> "CAM_1", para componer CAM_CAM_1_USER., _resolver_credenciales(), _sufijo_entorno(), build_rtsp_url(), Devuelve la URL RTSP para una cámara. Args: model: clave de RTSP_PATHS, p. ej.… (+21 more)
...
### Community 62 - "dron_robotat.py"
Cohesion: 0.10
Nodes (19): Estado, Opciones, parametros_firmware(), ramp_velocity(), Un Crazyflie sobre el Robotat: preflight, despegar, mover, aterrizar, parar.…, Qué hacer en vuelo: ("aterrizar" | "cortar", motivo), o None si todo va bien.…, `--param grupo.nombre=valor`, repetible, como diccionario ordenado., Acerca `actual` a `deseada` sin superar `accel · dt` por paso. (+11 more)

### Community 63 - "DroneUnit"
Cohesion: 0.09
Nodes (22): DroneUnit, mqtt_timestamp_is_older(), parse_mqtt_timestamp(), Pose, Crazyflie, datetime, Estado de un Crazyflie sobre el Robotat: pose MQTT, extpos, telemetría EKF y…, Prepara el EKF para posición externa con el commander high-level activo. (+14 more)

### Community 64 - "ScaleEstimator"
Cohesion: 0.13
Nodes (6): BodyFrame, Sistema de referencia anclado al cuerpo, en el instante de un frame., Cuán bien determinado está el eje lateral, en `[0, ~0.6]`. Es el ancho de…, Lleva puntos del marco de la cámara al marco del cuerpo., Escala corporal en vivo: mediana acumulada con calentamiento. Offline la escala…, ScaleEstimator

### Community 65 - "Primer vuelo del controlador de un dron sobre el Robotat"
Cohesion: 0.07
Nodes (28): Cuarto vuelo: `--ganancias mitad --velocidad 0.25` (15:26, Dron 2), Cámara de dos drones con el controlador nuevo (19:34): funciona, Dron 1 (15:06 y 15:07): no despega por batería, Dron 1 con batería nueva (15:15): tampoco levanta, Dual de las 18:55: el Dron 2 se apaga por batería, no por el Robotat, Empuje de hover medido (17:01 y 17:03, Dron 2), La oscilación, Lo que queda (+20 more)

### Community 66 - "analizar_sesion_robotat.py"
Cohesion: 0.16
Nodes (27): analyze_session(), cargar(), escribir_resumen(), figura_actitud(), figura_altura(), figura_bateria_empuje(), figura_error_y_mocap(), figura_trayectoria() (+19 more)

### Community 67 - "ReconocedorVocabulario"
Cohesion: 0.07
Nodes (24): AjustesPTZ, Dinamicos (DTW) + estaticos + paro, con dos modos excluyentes. Por frame hace…, La camara sigue al operador con su pan/tilt, como en el probador., Un frame de decision: `(aviso para el panel, pedir emergencia)`. Quien dice si…, ReconocedorVocabulario, Decision, MaquinaDeModos, Maquina de modos del vocabulario: que gesto vale en que modo. Sin dron. Es la… (+16 more)
...
### Community 69 - "DronSimulado"
Cohesion: 0.11
Nodes (10): DronSimulado, goto_duration_s(), Misma interfaz que `DronRobotat`, sin radio ni broker. Para `--dry-run`., Espera a que termine la orden en curso. False si no terminó a tiempo., Duración de un `go_to` proporcional a lo que hay que recorrer., Traduce `radio://<serial>/...` a `radio://<indice USB>/...`. Las URIs que ya…, resolve_serial_uris(), opciones() (+2 more)

### Community 70 - "calibration/__init__.py"
Cohesion: 0.14
Nodes (22): corner_centroid(), corner_extent(), Lado mayor del tablero como fracción del ancho de la imagen. Es **la medida que…, Centro del tablero en píxeles, para medir cobertura del cuadro., Calibración: intrínsecos, extrínsecos y matrices de proyección. Este subpaquete…, cargar_puntos(), _coherencia(), diagnosticar() (+14 more)
...
### Community 73 - "body_3d_rules.py"
Cohesion: 0.06
Nodes (46): bucle(), _contador_compacto(), dibujar_panel(), _falta_dos_manos(), main(), Counter, _que_falta(), Prueba del vocabulario 3D. Sin dron, sin radio y sin cflib. No importa… (+38 more)

### Community 74 - "extrinsics.py"
Cohesion: 0.15
Nodes (12): analizar_coherencia(), Coherencia, pose_relativa_de_un_par(), _poses_tablero(), Calibración extrínseca entre dos cámaras (estéreo). Da la posición y…, Resultado de comprobar que todos los pares describen la misma geometría., Pose del tablero en cada vista, resolviendo PnP., Pose de B respecto de A implicada por un solo par. Un tablero visto a la vez… (+4 more)

### Community 75 - "capturar_en_vivo"
Cohesion: 0.23
Nodes (14): capturar_en_vivo(), cargar_de_carpeta(), _es_vista_nueva(), main(), parse_args(), Namespace, ndarray, Path (+6 more)
...
### Community 78 - "Anclaje al marco Robotat con blanco rastreado por el MoCap"
Cohesion: 0.22
Nodes (11): Distincion: OptiTrack/MoCap vs camaras IP, Marcadores ArUco fijos medidos una vez, Anclaje al marco Robotat con blanco rastreado por el MoCap, Bola de color en el centroide de un cluster de marcadores, Tablero ChArUco rastreado como cuerpo rigido, LED-ring deck de Bitcraze como baliza, Verificacion obligatoria del frame rate real, Interferencia IR 850 nm contra el OptiTrack (+3 more)
...
### Community 80 - "RobotatCruzBackend"
Cohesion: 0.14
Nodes (11): opciones_desde_args(), Any, Uno o dos Crazyflies con el controlador nuevo, vistos como el backend de la…, Uno o dos `DronRobotat` con la interfaz del backend de la cruz., Velocidad ahora y parada programada; una orden nueva reprograma la parada., Paso del protocolo (dx, dy, dz, dyaw) como pulso de velocidad fluida., Desplazamiento pedido por el seguidor como velocidad: delta / periodo., `Opciones` de un dron a partir de los argumentos comunes de la cruz. (+3 more)

### Community 81 - "Frame"
Cohesion: 0.14
Nodes (11): anticipate(), Posición extrapolada `p + v·τ` para compensar la latencia del mocap., Promedio de una ventana de frames si el marker estuvo quieto., Se llama en el hilo MQTT con cada frame distinto; reenvía al EKF., Bloque de posición del EKF (`stateEstimate.x/y/z/yaw`)., stable_origin(), Frame, Un frame distinto del Robotat, en metros del marco global. (+3 more)

### Community 82 - "MocapReceiver"
Cohesion: 0.22
...
Cohesion: 0.14
Nodes (13): _args(), backend(), fixture, Namespace, Construccion del backend de hardware. Sin radios y sin volar. Existe por un…, Si falta un import en __init__, esto revienta aqui y no en el laboratorio., El segundo fallo del 2026-09-12: `reloj` importado dentro de __init__. Quedaba…, El panel lo llama en bucle; no puede depender de estado de la primera vez. (+5 more)

### Community 84 - "validate_step"
Cohesion: 0.14
Nodes (9): Move, Un paso relativo validado. Se construye con `validate_step`., Aplica los límites de un paso; lanza `DronError` si se exceden., Geocerca horizontal alrededor del origen y rango vertical absoluto., validate_step(), validate_target(), parametrize, TestObjetivo (+1 more)

### Community 86 - "HandGestureDetector"
Cohesion: 0.14
...
### Community 87 - "test_marker_follow_level.py"
Cohesion: 0.27
Nodes (7): pose(), Ancla del seguidor a la altura del marker (`activate(level=True)`)., ReceptorFalso, seguidor(), test_ancla_nivelada_con_dron_encima_del_marker(), test_ancla_nivelada_ignora_la_altura_del_dron(), test_ancla_tridimensional_sigue_igual()

### Community 88 - "flowdeck_flight.py"
Cohesion: 0.09
Nodes (29): Exclusión de flujo óptico y ToF en Robotat, configure_estimator(), Operaciones sobre un enlace Crazyflie que todos los controladores repiten.…, Pulso de `kalman.resetEstimation`., Prepara el Crazyflie para volar con posición externa del Robotat. Excluye la…, Corta los motores de uno o varios Crazyflie repitiendo el stop setpoint. Se…, reset_kalman(), stop_motors() (+21 more)
...
### Community 94 - "Arquitectura actual (docs/agents/architecture.md)"
Cohesion: 0.09
Nodes (29): Reduccion de ~21900 a ~14500 lineas sin perder funcionalidad de vuelo, Base de los registros CSV de sesión: archivo por corrida, reloj y análisis.…, connected_radios(), make_uri(), Identidad de las Crazyradio y de los Crazyflie del laboratorio. Concentra…, URI cflib para una antena (serial o índice USB) y un enlace., Seriales de las Crazyradio conectadas por USB, en mayúsculas., Elige una antena conectada. Con `requested` exige ese serial. Sin él toma la… (+21 more)
...
### Community 97 - "VueloRobotat"
Cohesion: 0.06
Nodes (20): `VueloRobotat`: la interfaz que espera el controlador por cámara, sin hardware., Imita `CameraMarkerFollower`: ancla fija y objetivo configurable., SeguidorFalso, TestInterfaz, TestOpciones, TestSeguirMarker, vuelo(), opciones_camara() (+12 more)
...
Cohesion: 0.08
Nodes (31): BaseException, describir_error(), Convierte la excepcion en algo que diga que hay que arreglar. Los tres fallos…, ControlPTZ, Registra la orden y vuelve de inmediato. `None` no hace nada., Para el hilo y garantiza que el motor queda detenido., Puente no bloqueante entre la politica y la camara. Uso:: with…, Una pasada del hilo. Separada para poder probarla sin hilos. (+23 more)

### Community 104 - "comparar_2d_3d.py"
Cohesion: 0.18
Nodes (17): entre_personas(), evaluar(), main(), por_sujeto(), ndarray, rasgos_2d(), rasgos_3d(), Compara 3D contra 2D sobre las tomas grabadas. Sin camara y sin dron. Responde… (+9 more)

### Community 105 - "wrap_deg"
Cohesion: 0.16
Nodes (9): fence_velocity(), plan_hold_segment(), Objetivo, yaw y duración del siguiente tramo continuo. La dirección se…, Simulación: la pose avanza un paso de `FLUID_LOOKAHEAD_S` por llamada., Ángulo equivalente en [-180, 180)., Quita de `vel` lo que sacaría a `pose` de la geocerca. Horizontal: si la…, wrap_deg(), Fuera del círculo por inercia, una tecla tangencial sigue moviendo. (+1 more)

### Community 107 - "test_diagnostico.py"
Cohesion: 0.08
Nodes (37): detectar_flanco(), ndarray, Latencia en segundos entre el destello y el primer frame iluminado. Args:…, evaluar_encuadre(), jitter_en_reposo(), ndarray, Cuántos frames tienen el cuerpo entero dentro del cuadro. Args: secuencia_norm:…, Ruido de posición de cada landmark con el sujeto quieto. Args: secuencia_px:… (+29 more)
...
### Community 110 - "PanelRobotat"
Cohesion: 0.12
Nodes (3): PanelRobotat, Any, Ventana con botones y teclado. Las órdenes corren en un hilo aparte.

### Community 111 - "marker_mocap.py"
Cohesion: 0.38
...
### Community 112 - "test_control_dron_robotat.py"
Cohesion: 0.23
Nodes (11): args(), parametrize, Línea de órdenes del panel: sin abrir Tk, radios ni broker., test_ganancias_fabrica_no_toca_nada(), test_ganancias_mitad_y_param_manda(), test_ganancias_robotat(), test_opciones_dron_1_por_defecto(), test_opciones_dron_2_con_ajustes() (+3 more)
...
### Community 115 - "PoseDetector"
Cohesion: 0.12
Nodes (6): _gesto_de_mano(), Vocabulario 3D de cuerpo entero, mas los dos gestos de mano del marker., ReconocedorCuerpo, PoseDetector, Convierte frames BGR en landmarks sin aplicar clasificación., Devuelve `(landmarks, world_landmarks)`, o `(None, None)`. `pose_landmarks`…

### Community 117 - "server.py"
Cohesion: 0.21
...
### Community 118 - "config/cameras.local.yaml (plantilla cameras.example.yaml)"
Cohesion: 0.16
Nodes (14): Dataset del vocabulario (54 tomas por persona), Gestos dinamicos por DTW (aplausos y secuencias), Prohibicion de comandos PTZ, config/cameras.local.yaml (plantilla cameras.example.yaml), integrity_check ArUco (DICT_4X4_50, 3 px), Precedencia de credenciales de camara, Coherencia entre pares: detectar camara movida durante la sesion, Integridad de la calibracion en tres capas (+6 more)

### Community 120 - "Comandos para probar los programas"
Cohesion: 0.14
...
### Community 121 - "grid_spacing"
Cohesion: 0.17
Nodes (13): Herramientas externas: Caliscope, aniposelib, Pose2Sim, Homografia del suelo: dominio de validez, Rectificar puntos antes de triangular (undistort_points), DLT de N vistas, Referencias: TemugeB/bodypose3d, Pose2Sim, aniposelib, Tres capas de robustez sobre el DLT, Verificacion metrica: triangular el tablero y medir la casilla, grid_spacing() (+5 more)
...
Cohesion: 0.08
Nodes (37): analyze_session(), main(), number(), Path, Genera gráficas y un resumen de la última sesión de dos drones., Crea figuras PDF para un CSV y devuelve la carpeta de salida., session_day(), session_output_dir() (+29 more)
...
### Community 138 - "tests/conftest.py"
Cohesion: 0.50
Nodes (3): _memoria_de_empuje_temporal(), fixture, Los tests de esta carpeta nunca escriben en `cache/` del repositorio.
...
### Community 141 - "probar_vocabulario.py"
Cohesion: 0.05
Nodes (64): Un paso del seguimiento. `True` si el frame se tomo con la camara girando. Va…, carpeta_de_hoy(), bucle(), como_hacerlo(), construir_guion(), dibujar(), encuadre(), main() (+56 more)

### Community 142 - "check_stream.py"
Cohesion: 0.33
...
### Community 143 - "CamaraPTZ"
Cohesion: 0.14
Nodes (9): CamaraPTZ, _parsear_respuesta(), Posicion y estado del motor. No mueve nada., `(pan, tilt, zoom)` o `None` si la camara no la reporta., `(modelo, numero de serie)` de la camara. El serie es lo unico que distingue de…, Detiene el movimiento en curso. La API pide el mismo `code` con el que se…, Traduce la salida de `Seguidor.decidir` en una peticion. Acepta cualquier…, `clave=valor` por linea -> diccionario. (+1 more)

### Community 144 - "Aplicaciones de apps/ y su prioridad"
Cohesion: 0.29
Nodes (6): main(), parse_args(), Namespace, Genera un tablero de ajedrez para calibración, a escala exacta, en PDF. El…, Aplicaciones de apps/ y su prioridad, Tablero de calibracion con regla de verificacion de 100 mm
...
### Community 147 - "Mapeo tridimensional con camaras (mapeo3d)"
Cohesion: 0.29
Nodes (8): Amcrest IP4M-1041B (x6, objetivo), Diagnostico RTSP: 401 vs 404, Red para las seis Amcrest, Wi-Fi 2.4 GHz contra el Crazyradio, Convencion de estado [implementado]/[parcial]/[planeado], Mapa de documentacion de mapeo3d, Configuracion previa de camaras (IR off, patrulla off, IP fija), Mapeo tridimensional con camaras (mapeo3d)
...
### Community 149 - "control_dron_robotat.py"
Cohesion: 0.32
Nodes (7): build_parser(), main(), opciones_desde_args(), ArgumentParser, Namespace, r"""Panel de teclado para un Crazyflie sobre el Robotat. Controlador nuevo, de…, Traduce la línea de órdenes; no abre radios ni broker.
...
### Community 151 - "video_tablero"
Cohesion: 0.14
Nodes (14): app_calibracion(), app_pose3d(), _fourcc(), _placa_tablero(), fixture, ndarray, parametrize, Path (+6 more)

### Community 152 - "seguidor.py"
Cohesion: 0.12
Nodes (13): decidir_con_gesto(), Orden, Politica de seguimiento: donde esta la persona -> hacia donde y cuan rapido.…, Lo que hay que pedirle al motor. `codigo == PARAR` es detenerse., Velocidad PTZ para un error dado, entre `velocidad_min` y `_max`. Justo en el…, Envoltorio de `Seguidor.decidir_con_gesto`, por comodidad de llamada., Cuanto puede durar un movimiento seguido, en segundos., Orden de parada incondicional, para el cierre del programa. (+5 more)

### Community 153 - "gui_pdf_capture.py"
Cohesion: 0.21
...
Cohesion: 0.13
Nodes (12): visibility como peso del DLT, presence aparte, Landmarks2D, pares_de_huesos(), ndarray, Convierte `HUESOS` en índices `(M, 2)` para `triangulation.metrics`.…, Landmarks de **una** imagen de **una** cámara. Args: xy: `(K, 2)` normalizado a…, `(K, 2)` en píxeles de la imagen original. Es lo que consume la triangulación,…, Peso `(K,)` para la triangulación ponderada. Es `visibility` a secas. MediaPipe… (+4 more)

### Community 155 - "Backend high-level de la cruz (cruz_highlevel_backend.py)"
Cohesion: 0.29
...
### Community 157 - "SupervisorSeparacion"
Cohesion: 0.22
Nodes (4): Any, Aterriza a los dos drones si en vuelo se acercan a menos de `minimo_m`., Una comprobación; devuelve la separación. Se puede llamar sin hilo., SupervisorSeparacion

### Community 158 - "Pipeline de gestos por vision: diseno (docs/agents/gesture_pipeline.md)"
Cohesion: 0.10
Nodes (28): config.py mezcla tres subsistemas (deuda tecnica), Dos sistemas de camaras separados: MoCap (dron) vs camaras IP (operador), Baseline 1: MediaPipe + DTW, Gate de engagement obligatorio antes de cualquier comando, Baseline 2: MediaPipe + HMM, Leave-One-Subject-Out (LOSO) / validacion estricta por sujeto, Modelo posterior: LSTM/GRU sobre landmarks normalizados, Fusion multicamara de decisiones (no reconstruccion 3D) (+20 more)

### Community 159 - "BancoDinamico"
Cohesion: 0.18
Nodes (7): BancoDinamico, Path, Plantillas y umbral de aceptacion, medidos sobre material real., Gestos que el banco puede emitir. `GESTO_RECHAZO` no es uno., Umbral que se le exige a `gesto`: el suyo si lo tiene, si no el global., `(gesto, distancia, margen, distancias por gesto)`. `gesto` es `None` si la…, test_el_banco_sobrevive_al_disco()

### Community 160 - "demo_pose3d.py"
Cohesion: 0.20
Nodes (14): main(), panel_3d(), panel_datos(), panel_pipeline(), parse_args(), Namespace, ndarray, Demostración en vivo del pipeline de pose 3D, con una sola cámara. Pensada para… (+6 more)
...
### Community 163 - "normalize_key"
Cohesion: 0.38
Nodes (3): normalize_key(), Event, Event

### Community 164 - "SessionRecording"
Cohesion: 0.22
...
### Community 167 - "Panel web UVG Drone Lab (index.html)"
Cohesion: 0.25
Nodes (9): Seguridad de hardware, control_dos_drones_camara.py (dos manos, dos drones), --dry-run: backend high-level simulado sin radio ni mocap, Servidor local sin TLS: token de control y comprobación de origen, web/server.py (HTTP y estáticos del panel), Controles de ensayo sin hardware (#demo-controls), Botón de paro de emergencia (#emergency), Panel web UVG Drone Lab (index.html) (+1 more)

### Community 168 - "Marker joystick: rigid body 64 en mocap/all"
Cohesion: 0.28
Nodes (9): Marker joystick: rigid body 64 en mocap/all, marker_input.py (marker 64 como joystick), Criterio de reutilización real en shared/, shared/robotat.py (broker MQTT y tópicos), paho-mqtt (MQTT / MoCap), Control por gestos de manos en el panel web, Joystick con marker en el panel web, Vista de cámara con manos anotadas (#camera-feed) (+1 more)

### Community 169 - "Reconocedor cuerpo 3D (MediaPipe Pose)"
Cohesion: 0.25
...
### Community 170 - "control_camara_dron1.py (controlador único por cámara)"
Cohesion: 0.23
Nodes (13): Protocolo de seguimiento del marker Robotat 65, marker_follow.py (seguimiento del marker 65), Categorías de controladores, --backend flowdeck (FlowDroneController de flowdeck_dual_backend.py), --backend mocap (posición absoluta, geofence, dry-run), control_camara_dron1.py (controlador único por cámara), CSV por frame y gráfica tiempo vs comandos por sesión, grafica_comandos.py (GraficaDeComandos) (+5 more)

### Community 173 - "DualFlightLogger"
Cohesion: 0.32
Nodes (3): Namespace, DualFlightLogger, test_connect_deja_los_dos_listos()

### Community 174 - "AGENTS.md — contrato para agentes de mapeo3d"
Cohesion: 0.22
Nodes (11): AGENTS.md — contrato para agentes de mapeo3d, Pipeline capture → pose → triangulation → io, Salida MQTT topico vision/pose3d, Triangulacion independiente del backend de pose, Marco de coordenadas del Robotat como salida unica, Paquete src/mapeo3d con lanzadores en apps/, PoseFrame3D (contrato de salida), MediaPipe Tasks API (1.0 elimino mp.solutions) (+3 more)

### Community 175 - "Opciones para bajar líneas de código"
Cohesion: 0.29
...
### Community 177 - "Montaje de camaras en las esquinas del Robotat"
Cohesion: 0.33
...
### Community 178 - "discover_marker_id.py"
Cohesion: 0.40
...
### Community 179 - "._configure_firmware"
Cohesion: 0.40
Nodes (4): guardar_empuje_memoria(), leer_empuje_memoria(), Path, Empuje de hover guardado del último vuelo de `nombre`, o None.

### Community 180 - "FollowGestureTests"
Cohesion: 0.31
Nodes (3): DummyLandmarks, FollowGestureTests, Clasificación de los dos gestos de seguimiento, sin cámara.

### Community 181 - "Un Crazyflie sobre el Robotat, desde cero"
Cohesion: 0.40
Nodes (5): Archivos, Juego validado y cómo se llegó a él, Qué cambia respecto al backend de la cruz, Resultados, Un Crazyflie sobre el Robotat, desde cero

### Community 182 - "shared/tests/conftest.py"
Cohesion: 0.50
Nodes (3): _memoria_de_empuje_temporal(), fixture, Los tests de esta carpeta nunca escriben en `cache/` del repositorio.

### Community 183 - "2026-09-17 Nucleo Robotat en shared.md"
Cohesion: 0.50
Nodes (3): Consecuencias, Contexto, Decisión

### Community 185 - "Mapa de documentacion para agentes (docs/agents/README.md)"
Cohesion: 0.40
...
### Community 188 - "test_triangulation.py"
Cohesion: 0.15
Nodes (15): angulo_utilizable(), ¿El ángulo entre rayos está en la zona donde la triangulación informa?, _P(), par_estereo(), fixture, ndarray, Pruebas del DLT, sin cámaras y sin calibración. Se define un punto 3D conocido,…, Los dos extremos son igual de malos: paralelo y colineal. (+7 more)

### Community 190 - "T-006 Controlador de un dron sobre el Robotat desde cero.md"
Cohesion: 0.40
Nodes (4): Bitácora, Contexto, Criterio de aceptación, Objetivo

### Community 191 - "2026-09-16.md"
Cohesion: 0.40
Nodes (4): Agentes, Bloqueos y dudas, Hecho hoy, Mañana
...
## Knowledge Gaps
- **195 isolated node(s):** `DummyLandmarks`, `form`, `names`, `directions`, `gestures` (+190 more)
...
- **Why does `Refactorizacion de septiembre de 2026 (docs/agents/refactor_2026-09.md)` connect `Arquitectura actual (docs/agents/architecture.md)` to `AGENTS.md: contrato canónico para agentes`, `body_3d_rules.py`, `BridgeError`, `marker_input.py`, `flowdeck_flight.py`, `Mapa de documentacion para agentes (docs/agents/README.md)`, `FlowDroneController`, `DroneUnit`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `canonicalizar()` connect `test_canonical.py` to `mapeo3d/pose/__init__.py`, `test_robust.py`, `probar_vocabulario.py`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `Mapeo tridimensional con camaras (mapeo3d)` connect `Mapeo tridimensional con camaras (mapeo3d)` to `demo_pose3d.py`, `AGENTS.md: contrato canónico para agentes`, `check_stream.py`, `AGENTS.md — contrato para agentes de mapeo3d`, `Anclaje al marco Robotat con blanco rastreado por el MoCap`, `check_connection.py`, `Aplicaciones de apps/ y su prioridad`, `config/cameras.local.yaml (plantilla cameras.example.yaml)`, `Mapa de documentacion para agentes (docs/agents/README.md)`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Are the 18 inferred relationships involving `Command` (e.g. with `HighLevelFlight` and `HighlevelCameraMarkerRuntime`) actually correct?**
  _`Command` has 18 INFERRED edges - model-reasoned connections that need verification._
- **Are the 13 inferred relationships involving `BridgeError` (e.g. with `HighLevelFlight` and `FlowCruzBackend`) actually correct?**
  _`BridgeError` has 13 INFERRED edges - model-reasoned connections that need verification._