...
def lowlevel_takeoff_and_hover(cf, mocap, target, hover_time, emergency):
    ...
    takeoff_deadline = (
        time.monotonic()
        + target[2] / MAX_VERTICAL_SPEED_M_S
        + 5.0
    )
...
def lowlevel_land(cf, mocap, target_xy, emergency):
    ...
    while time.monotonic() < deadline:
        ...
        vx = POSITION_KP_XY * (target_xy[0] - x)
        vy = POSITION_KP_XY * (target_xy[1] - y)
        horizontal_speed = math.hypot(vx, vy)
        if horizontal_speed > MAX_HORIZONTAL_SPEED_M_S:
            factor = MAX_HORIZONTAL_SPEED_M_S / horizontal_speed
            vx *= factor
...
def parse_args():
    # Permite probar cualquier Crazyflie individual sin editar este archivo.
    parser = argparse.ArgumentParser()
    parser.add_argument("--uri", default=URI, help="Crazyradio URI")
    parser.add_argument("--topic", default=MQTT_TOPIC, help="MoCap topic")
    ...
    try:
        ...
        with SyncCrazyflie(URI, cf=create_crazyflie()) as scf:
            ...
            if result == "emergency":
                ...
                lowlevel_land(
                    cf, mocap, (target_x, target_y), emergency
                )
