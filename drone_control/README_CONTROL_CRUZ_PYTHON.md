...
```powershell
python .\drone_control\control_dos_drones_cruz_botones.py --dry-run
```
...
```powershell
python .\drone_control\control_dos_drones_cruz_botones.py
```
...
```powershell
python .\drone_control\control_dos_drones_cruz_multiprocessing.py --single drone1
```

Para probar únicamente el Dron 2:

```powershell
python .\drone_control\control_dos_drones_cruz_multiprocessing.py --single drone2
```
...
`drone_control/datos_dos_drones/python_highlevel_cruz_*.csv`.
Al cerrar el programa genera automáticamente figuras PDF dentro de
`gráficas/python_highlevel_cruz_*`. Incluye altura, trayectoria XY, error de
