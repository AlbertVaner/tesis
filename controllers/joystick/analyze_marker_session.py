...
DATA_DIR = PROJECT_DIR / "results" / "data" / "marker"
GRAPH_DIR = PROJECT_DIR / "results" / "graphs" / "marker"
...
def main() -> None:
    ...
    path = args.csv
    if path is None:
        files = sorted(DATA_DIR.rglob("sesion_marker_*.csv"), key=lambda item: item.stat().st_mtime)
        if not files:
    ...
    out = GRAPH_DIR / session_day(path) / path.stem
