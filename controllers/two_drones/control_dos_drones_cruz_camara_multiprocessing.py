...
for directory in (MODULE_DIR, GESTURE_DIR, SHARED_DIR):
...
def camera_loop(backend: ProcessBackend, args: argparse.Namespace) -> None:
    ...
    output_dir = PROJECT_DIR / "results" / "data" / "dos_drones" / day
    ...
    try:
        with csv_path.open("w", newline="", encoding="utf-8") as csv_file:
            ...
            while True:
                ...
                last_displayed = displayed.copy()
                cv2.imshow(WINDOW_NAME, displayed)
                key = cv2.waitKey(1) & 0xFF
                if key == ord("p"):
                    path = save_cv_frame_pdf(last_displayed, "gui_control_camara_dos_drones")
                    print(f"GUI de cámara guardada en PDF: {path.resolve()}")
                if key == ord("q"):
                    break
    finally:
