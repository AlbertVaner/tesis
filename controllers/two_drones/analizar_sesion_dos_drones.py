...
DATA_DIR = PROJECT_DIR / "results" / "data" / "dos_drones"
GRAPH_DIR = PROJECT_DIR / "results" / "graphs" / "dos_drones"
...
def generate_battery_plot(source: Path) -> Path:
    ...
    output_dir = GRAPH_DIR / session_day(source)
    output_dir.mkdir(parents=True, exist_ok=True)
    output = output_dir / f"bateria_{source.stem}.pdf"
...
def main() -> None:
    ...
    source = args.csv
    if source is None:
        files = sorted(DATA_DIR.rglob("*.csv"), key=lambda path: path.stat().st_mtime)
        if not files:
